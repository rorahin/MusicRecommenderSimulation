...
The system asks a listener four questions: What is your favorite genre? What mood do you prefer? How energetic do you want the music to be, on a scale from 0 (very calm) to 1 (very intense)? Do you prefer acoustic or electronic-sounding music? Every song in the catalog is then graded on each of those four questions. Genre and mood are pass-or-fail: a song either matches the listener's preference exactly and earns full credit, or it earns nothing. Energy is scored by closeness — a song that is 0.10 away from the listener's target scores better than one that is 0.40 away. Acousticness works the same way: if a listener prefers acoustic sound, songs with high acousticness score well; if they prefer electronic production, songs with low acousticness score well.

The four sub-scores are combined into one final number using fixed weights: genre accounts for 40% of the total, mood for 30%, energy for 20%, and acousticness for 10%. Every song gets this final number, all 20 are sorted from highest to lowest, and the top five are returned. Each recommendation also comes with a plain-English explanation — for example, "genre matches your favorite (hip-hop), energy level is close to your target (0.80)" — so the listener can see exactly why a song was chosen.
...
The recommender was tested against five distinct user profiles: a late-night hip-hop worker seeking focused, high-energy tracks; a coffeehouse jazz listener preferring relaxed, acoustic-heavy music; a gym user seeking maximum-energy pop; and two adversarial profiles designed to expose scoring gaps — a moody electronic listener whose exact profile exists nowhere in the catalog, and a cold-start user with no genre or mood declared. The hip-hop and jazz profiles produced sensible rank-1 results (Bassline Therapy at 0.984 and Coffee Shop Stories at 0.989 respectively), but ranks 2–5 for both profiles quickly fell back to cross-genre songs ranked by energy and acousticness proximity rather than musical relevance, revealing how thin the catalog is beneath the top result. An unexpected pattern emerged during the moody electronic test: because no song in the catalog matches both "electronic" genre and "moody" mood simultaneously, the formula defaulted to a genre-match song with the wrong mood over a mood-match song with the wrong genre, exposing that the 0.40 genre weight will always override mood when the two signals conflict. A controlled weight shift experiment confirmed that the formula's top result is stable but that the weight distribution shapes diversity in ranks 2–5. The change involved two lines in `recommender.py` (the `score_song` function, line 121):

Before: `score = (0.40 * genre_score) + (0.30 * mood_score) + (0.20 * energy_score) + (0.10 * acousticness_score)`

After: `score = (0.20 * genre_score) + (0.30 * mood_score) + (0.40 * energy_score) + (0.10 * acousticness_score)`

With genre weight halved and energy weight doubled, Bassline Therapy remained rank 1 for the hip-hop profile (its genre and energy both still dominated), but ranks 2–5 shifted toward high-energy songs from diverse genres rather than staying within the hip-hop cluster, demonstrating that the weight distribution matters more for lower-ranked recommendations than for the top result.
